...
void Machine::show_time(ostream &os){
  ...
  //  if (((time_left_h-hour)*60 + (time_left_m-minute))>0 ) {os << ;}
...
void Machine::change_status(int t){
    status=t;
}
...
string Machine::next_available_time(){
    time_t ttime = time(0);
    tm *local_time = localtime(&ttime);
    int hour = local_time->tm_hour;
    int minute = local_time->tm_min;

    int temp_time = minute + (time_left_h-hour)*60 + (time_left_m-minute);
    timify clean(hour, temp_time);
    string a = to_string(clean.return_hour());
    string b = to_string(clean.return_minute());

     if (clean.return_minute() < 10){
      b = "0" + b;
    }
     if (clean.return_hour() < 10){
      a = "0" + a;
    }
    string output = a + ":" + b;
    return output;
...
//{
        for (int i=0; i<5; i++){
        if (id == building[i]->return_id()){          
            building[id]->change_status(1); //expand on it
            building[id]->change_time_left(left);
            //new stuff
            is >> waste;
            int timeleft = building[id]->return_time();
...
void Washer::scheduled(){
    time_left_m +=20;
}


void Dryer::scheduled(){
    time_left_m +=40;
}
